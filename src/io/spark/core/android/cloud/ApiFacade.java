...
public class ApiFacade {
	...
	public void digitalTest(String coreId, String pinId, DigitalValue oldValue) {
		TinkerReadValueReceiver receiver = new TinkerReadValueReceiver(handler,
				TinkerResponse.REQUEST_TYPE_READ, coreId, pinId,
				TinkerResponse.RESPONSE_TYPE_DIGITAL, oldValue.asInt());
		...
		args.putString("params", pinId);
//getdatatest
		SimpleSparkApiService.post(ctx, new String[] { "devices", coreId, "digitalread1" },
				args, receiver, null);
	}
			...
			if (statusCode == HttpStatus.SC_OK) {
				try {
					...
					if (json.has("return_value")) {
						...
						//Toaster.s(ctx, "Re-flashing " + newPinValue);
	...
	class TinkerGetValueTest extends BaseTinkerValueReceiver {

		public TinkerGetValueTest(Handler handler, int requestType, String coreId,
				String pinId, int valueType, int oldValue) {
			super(handler, requestType, coreId, pinId, valueType, oldValue);
		}
